...
class AuraShell {
    ...
    private var taskManager = AppSwitcher() // Наш AuraTaskManager
    ...
    private func forwardTouchToActiveLayer(x: Float, y: Float, event: TouchEvent) {
        ...
        case .appRunning:
            if let app = activeApplication as? CameraApp {
                ...
                app.handleTouch(x: x, y: y, event: event)
            } else if let app = activeApplication as? AuraSurfApp {
                app.handleTouch(x: x, y: y, event: event)
            } else if let app = activeApplication as? AuraPhotosApp {
                app.handleTouch(x: x, y: y, event: event)
            }
        }
    }
